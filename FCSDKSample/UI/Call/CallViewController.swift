...
class CallViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        callView.endButton.addTarget(self, action: #selector(endButtonTapped), for: .touchUpInside)
    }
    ...
    @objc private func endButtonTapped(_ button: UIButton) {
        callView.state = .ending
        if !viewModel.endCall() {
            callView.state = .initialised
        }
    }
}
